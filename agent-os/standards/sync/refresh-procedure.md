...
- An **instance alias** — `v1` (Old instance) or `v2` (New instance). **A refresh runs for exactly one instance at a time.** To sync both, run the whole procedure twice.
- `vault/<INST>/` (current state — workflow + resource + changelog notes for that instance)
- The live n8n instance for that alias, reached via its **REST API** (no MCP server required)
- This run's date in UTC (used to name the changelog file)

## Instance selection (parameterizes the whole runbook)

The vault holds one isolated subtree per instance. n8n workflow IDs are only unique *within* an instance, so **every path and API call below is scoped to the chosen alias**. Throughout this runbook:

- `<INST>` = the chosen alias (`v1` | `v2`). Set it once: `INST=v1`.
- Every `scripts/n8n-api.sh` call takes the alias as its **first argument**: `scripts/n8n-api.sh "$INST" '/api/v1/...'`.
- Every `vault/...` path means `vault/<INST>/...` — e.g. the cache is `vault/$INST/_cache/`, notes are `vault/$INST/workflows/` and `vault/$INST/resources/<type>/`, changelogs are `vault/$INST/changelogs/`.
- The renderer is invoked with the selector: `scripts/render-vault.sh --instance "$INST" …`.

Never render or write into the other instance's subtree. The `_templates/` directory at the vault root is shared and instance-agnostic.
...
- Every fetched workflow lands in `vault/<INST>/_cache/workflows/<n8n_id>.json` (gitignored)
- Resource extraction, fingerprint computation, and node inspection all happen via `jq` programs under `scripts/jq/`
- Claude reads only the small structured outputs of those programs — never `cat` / `Read` against `vault/<INST>/_cache/workflows/*`
...
- Updated / created notes under `vault/<INST>/workflows/` and `vault/<INST>/resources/<type>/`
- Optionally: `vault/<INST>/changelogs/YYYY-MM-DD.md` (only if at least one workflow's fingerprint changed, was new, or went away)
...
```sh
# First arg is the instance alias (v1|v2); always quote endpoint paths —
# ? and & trigger zsh globbing if unquoted.
...
1. Validates the instance alias against an allowlist (`v1`|`v2`) **before** using it, then resolves that instance's auth from `./.env` (`N8N_API_URL_<ALIAS>` / `N8N_API_KEY_<ALIAS>`). For `v1` it falls back to legacy unsuffixed `N8N_API_URL`/`N8N_API_KEY` and then `../n8n_claude/.mcp.json`; `v2` requires `.env`.
2. Calls curl with `X-N8N-API-KEY` + `Accept: application/json`
3. Uses `-fsS` so 4xx/5xx exits non-zero (no silent partial data)

The agent only ever passes the alias — never a URL or key — so credentials stay out of the command line and transcript.
...
```sh
: "${INST:?set INST=v1 or INST=v2 first}"
mkdir -p vault/$INST/_cache/workflows /tmp/n8n-pages
...
while :; do
  ...
  scripts/n8n-api.sh "$INST" "$url" > /tmp/n8n-pages/page-$page.json

  # Split each workflow into vault/$INST/_cache/workflows/<id>.json
  jq -c '.data[]' /tmp/n8n-pages/page-$page.json > /tmp/n8n-pages/page-$page.jsonl
  while IFS= read -r wf; do
    id=$(printf '%s' "$wf" | jq -r '.id')
    printf '%s' "$wf" > "vault/$INST/_cache/workflows/$id.json"
  ...
  jq -c '.data[] | {
    ...
    createdAt, updatedAt,
    tags: (.tags // [] | map(.name))
  }' /tmp/n8n-pages/page-$page.json >> vault/$INST/_cache/list.jsonl
...
wc -l vault/$INST/_cache/list.jsonl
```

Build `N = {n8n_id → metadata}` by reading `vault/$INST/_cache/list.jsonl` (small — one compact JSON per line).

### Step 2 — Enumerate workflow notes in the vault

Use `Bash` (`ls vault/$INST/workflows/*.md` or `find vault/$INST/workflows -name '*.md' -not -name '.gitkeep'`) and `Read` each note. Parse the YAML frontmatter from the top.
...
```sh
test -s vault/$INST/_cache/workflows/"$ID".json || \
  scripts/n8n-api.sh "$INST" "/api/v1/workflows/$ID" > vault/$INST/_cache/workflows/"$ID".json

jq '{id, name, nodes: (.nodes | length), updatedAt}' < vault/$INST/_cache/workflows/"$ID".json
```
...
jq -c -f scripts/jq/extract-triggers.jq     < vault/$INST/_cache/workflows/"$ID".json
jq -c -f scripts/jq/extract-databases.jq    < vault/$INST/_cache/workflows/"$ID".json
jq -c -f scripts/jq/extract-llm-models.jq   < vault/$INST/_cache/workflows/"$ID".json
jq -c -f scripts/jq/extract-env-vars.jq     < vault/$INST/_cache/workflows/"$ID".json
jq -c -f scripts/jq/extract-custom-nodes.jq < vault/$INST/_cache/workflows/"$ID".json
```
...
### Step 6.5 — Update `vault/$INST/index.md`

`vault/$INST/index.md` is this instance's orientation note (see `notes/frontmatter-schema.md` → "Index note"). Its auto block surfaces live section counts and wikilinks; refresh keeps it current. (The top-level `vault/index.md` is a separate hand-maintained router across instances — refresh never touches it.)
...
Counts come from disk (this instance's subtree), not from `vault/$INST/_cache/list.jsonl` (which mirrors n8n state). The index is a view of the vault subtree.

```sh
workflows=$(ls vault/$INST/workflows/*.md 2>/dev/null | wc -l | tr -d ' ')
changelogs=$(ls vault/$INST/changelogs/*.md 2>/dev/null | wc -l | tr -d ' ')
...
for d in vault/$INST/resources/*/; do
  cat=$(basename "$d")
...
- **If `vault/$INST/index.md` does not exist** → use `Write` with the full file content (frontmatter + auto block + empty manual stub) per `vault/_templates/index.md`. This writes the **per-instance** index; the top-level `vault/index.md` is a hand-maintained router across instances — do not overwrite it here.
...
**Never use `Write` on an existing `vault/$INST/index.md`** — it would clobber the manual block, same as any other vault note.
...
---
type: index
instance: <INST>            # v1 | v2 — which instance this subtree mirrors
...
If any workflow was added, modified, removed, or any taxonomy gap fired, write `vault/$INST/changelogs/<today>.md` per `sync/changelog-format.md`.
...
- `Bash` (`ls`, `find`, `grep`) — enumerating notes under `vault/$INST/`
- `Read` — loading **existing notes** (so the manual block can be captured before any write). **Never** to read `vault/$INST/_cache/workflows/*.json`.
...
**Do not read raw workflow JSON into context.** See `sync/large-workflow-handling.md` — Claude should never `Read` or `cat` files under `vault/<INST>/_cache/workflows/`. Use `jq` programs under `scripts/jq/` exclusively.
...
- **`scripts/n8n-api.sh` exits 2:** either an **unknown instance alias** (must be `v1` or `v2`) or **no credentials** for that instance. For missing credentials, tell the user to `cp .env.example .env` and fill in `N8N_API_URL_V1`/`N8N_API_KEY_V1` and/or `N8N_API_URL_V2`/`N8N_API_KEY_V2` (v1 also falls back to legacy unsuffixed vars and the sibling `.mcp.json`; v2 requires `.env`). Do not attempt to read `.env` directly — it's blocked by the user-level hook.
