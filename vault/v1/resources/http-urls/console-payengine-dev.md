...
- [[../../workflows/-managed-by-spartak-ai-agent-partner-merchant-bulk-import-sync-sandbox-|[Managed by Spartak AI Agent] Partner Merchant Bulk Import Sync (SANDBOX)]] — `POST https://console.payengine.dev/api/merchant/{{ $json.data?.id ?? $json.id }}/gateways` — node "Create Gateway" (id `create-gateway`)
