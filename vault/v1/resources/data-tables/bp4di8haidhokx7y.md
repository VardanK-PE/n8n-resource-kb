---
type: data-table
instance: v1
resource_id: "BP4Di8haiDhOKX7Y"
current_name: "Dispute - Elavon Attachments"
aliases: ["Dispute - Elavon Attachments","Dispute - Orphaned Email Attachments"]
auto_generated_at: 2026-08-19T19:25:44Z
---

<!-- auto:start -->

# Dispute - Elavon Attachments

- **Resource id (canonical):** `BP4Di8haiDhOKX7Y`
- **Current name:** Dispute - Elavon Attachments
- **Historical aliases:**
  - Dispute - Elavon Attachments
  - Dispute - Orphaned Email Attachments
- **Path:** `/projects/z00SrPPWOSGROMFl/datatables/BP4Di8haiDhOKX7Y`
- **Table ID:** `BP4Di8haiDhOKX7Y`

## Used by

- [[../../workflows/dispute-case-handler|Dispute - Case Handler]] — node "Get Pending Attachments" (id `82a30a5c-56ab-47fd-bd3e-7692a6d2d5bc`)
- [[../../workflows/dispute-case-handler|Dispute - Case Handler]] — node "Update row(s)" (id `7c8c8c7d-5104-46b4-8ea0-ff9fa60f1d31`)
- [[../../workflows/dispute-case-handler|Dispute - Case Handler]] — node "Update row(s)2" (id `da9aa254-df87-488e-8fe2-b7403995277b`)
- [[../../workflows/dispute-create-report|Dispute - Create Report]] — node "Get row(s)" (id `a0006edd-caa6-4485-b56b-5c5d106d4c0e`)
- [[../../workflows/dispute-create-report|Dispute - Create Report]] — node "Get row(s)1" (id `280d83ee-2935-4682-ac72-e15e712e7642`)
- [[../../workflows/dispute-create-report|Dispute - Create Report]] — node "Get row(s)2" (id `1cb5619c-924c-4294-8ccd-8a5a537a0e18`)
- [[../../workflows/dispute-create-report|Dispute - Create Report]] — node "Get row(s)5" (id `98d44034-82b6-46b4-8c9b-5a7fc53c9c87`)
- [[../../workflows/dispute-process-elavon-attachments|Dispute - Process Elavon attachments]] — node "Delete row(s)" (id `ace14950-946f-4a34-be5e-1b1bbc4be157`)
- [[../../workflows/dispute-process-elavon-attachments|Dispute - Process Elavon attachments]] — node "Get row(s)1" (id `b1111251-3562-4aa6-b45f-75778ebe8856`)
- [[../../workflows/dispute-process-elavon-attachments|Dispute - Process Elavon attachments]] — node "Get row(s)2" (id `8a3ae92f-cd6a-41eb-9aa6-73f028490621`)
- [[../../workflows/dispute-process-elavon-attachments|Dispute - Process Elavon attachments]] — node "Get row(s)4" (id `2573a89e-3481-442e-a04f-88ee91db91af`)
- [[../../workflows/dispute-process-elavon-attachments|Dispute - Process Elavon attachments]] — node "Get row(s)6" (id `deffa5a2-fb60-4719-a882-dd9dc9e9ac6e`)
- [[../../workflows/dispute-process-elavon-attachments|Dispute - Process Elavon attachments]] — node "If row does not exist" (id `40b474a3-0947-41e0-b82e-feeb6c234b6a`)
- [[../../workflows/dispute-process-elavon-attachments|Dispute - Process Elavon attachments]] — node "Insert row" (id `c2834559-3a29-41d1-8dd8-db8f663097b8`)
- [[../../workflows/dispute-process-elavon-attachments|Dispute - Process Elavon attachments]] — node "Upsert row(s)" (id `9e5d5781-9e60-4b9e-8b0d-fe8e06b6aa20`)
- [[../../workflows/dispute-process-elavon-attachments|Dispute - Process Elavon attachments]] — node "Upsert row(s)1" (id `bb0a1deb-8a7b-4e20-be00-73257961ef2c`)
- [[../../workflows/dispute-process-elavon-attachments|Dispute - Process Elavon attachments]] — node "Upsert row(s)2" (id `466d503f-f3d7-49d5-ad20-8ecf9be01c14`)
- [[../../workflows/opsinternalbot-disputes-backup-original-|OpsInternalBot - Disputes [Backup original]]] — node "Find transaction attachments" (id `cc107062-df82-4dfa-966b-89f8e4a4a5e7`)
- [[../../workflows/opsinternalbot-disputes-backup-original-|OpsInternalBot - Disputes [Backup original]]] — node "Get row(s)" (id `3b2f6b6f-262d-4829-bbb5-89559ef96c74`)
- [[../../workflows/opsinternalbot-disputes-backup-original-|OpsInternalBot - Disputes [Backup original]]] — node "Get row(s)1" (id `1830a491-988a-4ee4-b509-8c24a83eaaa4`)
- [[../../workflows/opsinternalbot-disputes-backup-original-|OpsInternalBot - Disputes [Backup original]]] — node "Get row(s)2" (id `e517b182-086c-435a-bf6b-fe483537b321`)
- [[../../workflows/opsinternalbot-disputes-backup-original-|OpsInternalBot - Disputes [Backup original]]] — node "Get row(s)5" (id `0d7443a5-cdc8-4673-b77e-dc326fb7617d`)
- [[../../workflows/opsinternalbot-disputes-backup-original-|OpsInternalBot - Disputes [Backup original]]] — node "Update row(s)" (id `dd6bc462-1aee-4877-964c-eb95eb333371`)
- [[../../workflows/opsinternalbot-disputes-v2|OpsInternalBot - Disputes v2]] — node "Find transaction attachments" (id `e9aa2313-3520-49f2-9f38-f3fd5b514db6`)
- [[../../workflows/opsinternalbot-disputes|OpsInternalBot - Disputes]] — node "Get row(s)" (id `ede24c4a-913c-4bc2-9af8-86d7a999e5ac`)
- [[../../workflows/opsinternalbot-disputes|OpsInternalBot - Disputes]] — node "Get row(s)1" (id `776b9c15-76c1-4bec-b654-5a26adb0038b`)
- [[../../workflows/opsinternalbot-disputes|OpsInternalBot - Disputes]] — node "Get row(s)2" (id `109c1903-42ac-4ca7-bac2-5f783e3850ba`)
- [[../../workflows/opsinternalbot-disputes|OpsInternalBot - Disputes]] — node "Get row(s)5" (id `5cbbfac2-f660-4cbb-b568-1d2a6893b3a9`)

<!-- auto:end -->

<!-- manual:start -->

<!-- Add owner, criticality, runbook URL, rotation cadence, etc. -->

<!-- manual:end -->
